...
class PlotCanvas(FigureCanvasQTAgg):
    ...
    def plot_scatter_on_axes2(self, x_data, y_data, facecolors, edgecolors, linewidth, label, size, alpha=1.0):
        
        scatter_obj = self.ax2.scatter(
            x_data, y_data, facecolors=facecolors, edgecolors=edgecolors, 
            linewidth=linewidth, label=label, s=size, alpha=alpha
        )
        self.ax2.legend()
        self.draw()
    ...
    def plot_line_on_axes2(self, x_data, y_data, linetype, color, label, linewidth, alpha=1.0):
        
        line_obj, = self.ax2.plot(
            x_data, y_data, linetype, color=color, label=label, linewidth=linewidth, alpha=alpha
        )
        self.ax2.legend()
        self.draw()
