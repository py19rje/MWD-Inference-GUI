...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QPushButton, QVBoxLayout, QHBoxLayout, QWidget, QFileDialog,
                             ...
                             QCheckBox, QComboBox, QFormLayout, QListWidget, QListWidgetItem)
...
from modules.GPCStatsWindow import GPCStatsWindow
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.select_model_button = QPushButton('Select NN Models', self)
        ...
        self.predicted_stats_window = StatsWindow("Predicted MWD Stats", {
            "N/A": [self.Est_Mn_label.text(), self.Est_Mw_label.text(), self.Est_PDI_label.text()]
        }, parent=self)

        self.gpc_stats_window = StatsWindow("GPC Stats", {
            "N/A": [self.GPC_Mn_label.text(), self.GPC_Mw_label.text(), self.GPC_PDI_label.text()]
        }, parent=self)
    ...
    def show_predicted_stats(self):
        ...
        self.model_stats_dict = getattr(self, 'model_stats_dict', None)
        if self.model_stats_dict is None:
    ...
    def show_gpc_stats(self):
        ...
        self.gpc_stats_window = GPCStatsWindow("GPC Stats", [
            self.GPC_Mn_label, self.GPC_Mw_label, self.GPC_PDI_label], parent=self)
        self.gpc_stats_window.show()
        
    def update_predicted_stats(self, curves_dict, raw_dict=None, pdi_dict=None):
        self.model_stats_dict = {}
        mn_list, mw_list, pdi_list = [], [], []
        def format_e(n):
            a = '%E' % n
            return a.split('E')[0].rstrip('0').rstrip('.') + ' E' + a.split('E')[1]
        if self.class_to_use == 2:  # Bidisperse
            mwS_list, mwL_list, pdiS_list, pdiL_list, phiS_list, phiL_list = [], [], [], [], [], []
            for label, curve in curves_dict.items():
                ...
                PDIS = None
                PDIL = None
                if raw_dict and label in raw_dict:
                    raw = raw_dict[label]
                    if isinstance(raw, (list, np.ndarray)) and len(raw) >= 3:
                        ...
                        ZS_pred = ZL_pred = phiL_pred = phiS_pred = float('nan')
                else:
                    ZS_pred = ZL_pred = phiL_pred = phiS_pred = float('nan')
                if pdi_dict and label in pdi_dict:
                    pdi = pdi_dict[label]
                    if isinstance(pdi, (list, np.ndarray)) and len(pdi) >= 2:
                        PDIS = pdi[0]
                        PDIL = pdi[1]
                    else:
                        PDIS = PDIL = float('nan')
                    ...
                    PDIS = PDIL = float('nan')
                MwS = ZS_pred * getattr(self, 'M_e', 1)
                MwL = ZL_pred * getattr(self, 'M_e', 1)
                self.model_stats_dict[label] = [
                    f"Component 1 (Mw): {phiS_pred*100:.2f}% {format_e(Decimal(MwS))} (g/mol)",
                    f"Component 2 (Mw): {phiL_pred*100:.2f}% {format_e(Decimal(MwL))} (g/mol)",
                    f"PDI 1: {PDIS:.2f}, PDI 2: {PDIL:.2f}"
                ]
                mwS_list.append(MwS)
                mwL_list.append(MwL)
                pdiS_list.append(PDIS)
                pdiL_list.append(PDIL)
                phiS_list.append(phiS_pred)
                phiL_list.append(phiL_pred)
            mean_MwS = format_e(Decimal(np.nanmean(mwS_list)))
            mean_MwL = format_e(Decimal(np.nanmean(mwL_list)))
            mean_PDIS = f"{np.nanmean(pdiS_list):.2f}"
            mean_PDIL = f"{np.nanmean(pdiL_list):.2f}"
            mean_phiS = np.nanmean(phiS_list)
            mean_phiL = np.nanmean(phiL_list)
            self.model_stats_dict["Mean"] = [
                f"Component 1 (Mw): {mean_phiS*100:.2f}% {mean_MwS} (g/mol)",
                f"Component 2 (Mw): {mean_phiL*100:.2f}% {mean_MwL} (g/mol)",
                f"PDI 1: {mean_PDIS}, PDI 2: {mean_PDIL}"
            ]
        else:
            for label, curve in curves_dict.items():
                # Use universal space if set
                x_data = np.log(m) 
                Mn = 1 / np.trapz(curve * np.exp(-x_data), x=x_data)
                Mw = np.trapz(curve * np.exp(x_data), x=x_data)
                PDI = Mw / Mn
                ...
                pdi_list.append(PDI)
            if len(mn_list) > 1:
                mean_Mn = format_e(Decimal(np.nanmean(mn_list)))
                mean_Mw = format_e(Decimal(np.nanmean(mw_list)))
                mean_PDI = f"{np.nanmean(pdi_list):.2f}"
    ...
    def select_model(self):
        ...
        model_list.setSelectionMode(QListWidget.MultiSelection)
        for name in display_names:
            item = QListWidgetItem(name)
            model_list.addItem(item)
        if self.app.is_dark:
            model_list.setStyleSheet(themes['dropdown_dark'])
        else:
    ...
    def make_prediction(self):
        ...
        for idx, (model, model_file) in enumerate(zip(self.models, loaded_model_names)):
            color = PREDICTION_COLORS[idx % len(PREDICTION_COLORS)]
            try:
                with torch.no_grad():
                    prediction = torch.abs(model(X_val_tensor))
    ...
    def revert_clean_pred(self):
        if not self.cleaned_pred:
            ...
            return
        for label in self.original_predictions_dict.keys():
            self.canvas.remove_single_plot(f"Predicted {label} (cleaned)")
            
        for idx, (label, curve) in enumerate(self.original_predictions_dict.items()):
            color = PREDICTION_COLORS[idx % len(PREDICTION_COLORS)]
    ...
    def PDI_change(self): 
        ...
        elif self.class_to_use == 2:  # Bidisperse
        ...
        if self.class_to_use == 1:  # Mono
            ...
            for idx, (label, pred) in enumerate(self.predictions_dict.items()):
                ...
                ZS_pred = 10**float(pred["raw"][0])
                ZL_pred = 10**float(pred["raw"][1])
                phiL_pred = float(pred["raw"][2])
                phiS_pred = 1 - phiL_pred
                ...
                pred_MWD = phiL_pred * lognormal(self.z, mean_ZL_pred, sigmaL) + phiS_pred * lognormal(self.z, mean_ZS_pred, sigmaS)
                pred_MWD = pred_MWD / np.trapz(pred_MWD, x = np.log(self.z))
                new_curves[label] = pred_MWD
    ...
    def change_frequency_range(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            try:
                ...
                return
            univ_space = self.univ_space
            self.clear_rheo_plot()
            self.univ_space = univ_space
            if self.univ_space:
